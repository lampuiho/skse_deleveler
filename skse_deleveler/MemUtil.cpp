...
void GetModuleLen(HANDLE hProcess, HMODULE hModule, size_t* addr, size_t* len){
	MODULEINFO info;
	GetModuleInformation(hProcess, hModule, &info, sizeof(MODULEINFO));
	*len = info.SizeOfImage;
	*addr = (size_t)info.lpBaseOfDll;
}

FARPROC GetRemoteProcAddress(HANDLE hProcess, HMODULE hModule, LPCSTR lpProcName, UINT Ordinal, BOOL UseOrdinal){
	BOOL Is64Bit = FALSE;
